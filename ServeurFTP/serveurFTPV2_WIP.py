...
def agent(socket_c):
    ...
    while True:
        ...
        if len(message) == 0:  # Client KO
            ...
            elif isConnected and commande == "RNFR":
                if verbose:
                    "Le client veut renommer un fichier"
            ...
            elif isConnected and (commande == "CWD" or commande == "XCWD"):
                if verbose:
                    print "Le client veut changer de repertoire"
                directory = message.split(" ")[1]

                #try:
                Utiliser os.getcwd(), le stocker dans une variable, et le passer au constructeur de Popen ?
                cd = subprocess.Popen(['cd', directory], stdout=subprocess.PIPE)
                data = cd.communicate()[0]
                verif = subprocess.Popen(['echo', "$?"], stdout=subprocess.PIPE)
                data = verif.communicate()[0]
                #except Exception:
                #   verif = 1

                if int(verif) == 0:
                    socket_c.send("250 OK.\r\n")
                    if verbose:
                        print "Changement de repertoire OK"
                    On modiife donc la valeur de osDir
                else:
                    socket_c.send("550 Impossible de changer de repertoire\r\n")
                    if verbose:
                        print "Changement de repertoire KO"
            """

            # TODO user sans arguments == renvoie l'username.
            # cf http://www.nsftools.com/tips/RawFTP.htm
